...
这就和蔡汀常数（Chaitin's Constant）相关了。

蔡汀常数的定义为无前缀（prefix-free）的图灵机中可停机的图灵机的占比，在我们的等价要求下，我们所讨论的映射其实就是一种无前缀图灵机。

而蔡汀常数的计算有一个相对来说更加直观的表达：它是所有可停机图灵机的随机涌现概率的求和。

这也就是说，蔡汀常数随着底层图灵机的运行空间的大小的增加，也就是可接受输入字符串的长度上限的增加，是一起增加的。
...
甚至于，根据蔡汀的不完全定理，我们都不可能比较两个足够复杂的能力或者说规律的 K 氏长度到底哪个更短从而更容易出现。
